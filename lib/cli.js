...
function newApp(argv) {
  return function () {
    ...
    return clusternator.newApp(clusterName, app, ec2Config).then(function (data) {
      console.log(data);
    }, util.errLog).then(null, util.errLog);
    //TODO REMOVE THAT
  };
}
...
function circleCIBuild(argv) {
  ...
  return function () {
    ...
    return circleCIClient.clusternate(config).then(function (resp) {
      console.log(resp);
    }, log.error);
  };
}
...
function createAppDefinition() {
  ...
  return function () {
    var defaultAppPath = path.resolve(__dirname, '../examples/DEFAULT.json');
    var defaultApp = JSON.parse(fs.readFileSync(defaultAppPath, 'utf8'));
