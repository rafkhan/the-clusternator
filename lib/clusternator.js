...
function updateApp(clusterName, appDef) {
  ...
  return clusterManager.describeCluster(clusterName).then(R.prop('clusterArn'), q.reject).then(clusterArn => {
    console.log('Initiating cleanup on', clusterArn);
    return clusterArn;
  }, q.reject).then(taskServiceManager.deleteAppOnCluster, q.reject).then(args => {
    var serviceNames = R.map(R.prop('serviceName'), args);
    console.log('Deleted services', serviceNames);
    return args;
  }, q.reject).then(loadNewApp, q.reject).then(services => {
