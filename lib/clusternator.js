...
function updateApp(clusterName, appDef) {
  console.log('Updating', appDef.name, 'on', clusterName, 'with app definition', '"' + appDef.name + '"');

  function loadNewApp() {
    return taskServiceManager.createAppOnCluster(clusterName, appDef);
  }

  return clusterManager.describeCluster(clusterName).then(R.prop('clusterArn'), q.reject).then(function (clusterArn) {
    console.log('Initiating cleanup on', clusterArn);
    return clusterArn;
  }, q.reject).then(taskServiceManager.deleteAppOnCluster, q.reject).then(function (args) {
    var serviceNames = R.map(R.prop('serviceName'), args);
    console.log('Deleted services', serviceNames);
    return args;
  }, q.reject).then(loadNewApp, q.reject).then(function (services) {
...
function newApp(clusterName, appDef, ec2Config) {
  ...
  return clusterManager.createCluster(clusterParams).then(buildEC2Instance, util.errLog).then(function () {
    return taskServiceManager.createAppOnCluster(clusterName, appDef);
  });
}
