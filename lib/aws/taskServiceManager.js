...
function getTaskServiceManager(ecs) {
  ...
  function deleteAllServicesOnCluster(clusterArn) {
    ...
    function stopService(serviceArn) {
      return updateService(clusterArn, serviceArn, { desiredCount: 0 }).then(R.prop('service'), q.reject);
    }
    ...
    function destroyService(serviceArn) {
      return deleteService(clusterArn, serviceArn).then(R.prop('service'), q.reject);
    }
    ...
    return listServices(clusterArn).then(R.prop('serviceArns'), q.reject).then(stopAllServices, q.reject).then(R.map(R.prop('serviceArn')), q.reject).then(destroyAllServices, q.reject);
  }
  ...
  function createTasksAndServicesOnCluster(clusterArn, appDef) {
    ...
    function createTaskAndService(task) {
      return taskDefinitionManager.createTaskDefinition(task).then(function (t) {
        console.log('Created task', t.taskDefinition.taskDefinitionArn);
        return t;
      }).then(createServiceFromTaskAndStart, q.reject);
    }

    var taskDefPromises = R.map(createTaskAndService, appDef.tasks);
