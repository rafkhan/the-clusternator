...
function getSubnetManager(ec2, vpcId) {
  ...
  function destroy(subnetId, pid) {
    return describe().then(function (list) {
      ...
      if (!isValid) {
        throw new Error('No Clusternator Tagged Subnet Available For ' + 'Destruction With SubnetId: ' + subnetId);
      }
  ...
  function create(params) {
    if (!params || !params.pid) {
      throw new Error('subnetManager.create requires a params object with a ' + 'pid value (project id)');
    }
    var awsParams = {
      VpcId: vpcId,
      CidrBlock: params.CidrBlock || constants.AWS_DEFAULT_CIDR,
      AvailabilityZone: params.AvailabilityZone || constants.AWS_DEFAULT_AZ
    };
